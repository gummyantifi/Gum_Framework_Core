...
AddEventHandler("gum:SelectedCharacter", function(charid)
	Citizen.CreateThread(function()
		...
		for k,v in pairs(Config.Locations) do		
			...
			Citizen.InvokeNative(0x9CB1A1623062F402, blips, Config.Language[1].text)
		end
		while true do
...
RegisterNUICallback('add_money', function(data, cb)
	if spam_protect == false then
		if tonumber(data.value) >= 0 then
			...
			exports['gum_notify']:DisplayLeftNotification(Config.Language[1].text, Config.Language[3].text, 'money', 3000)
		end
	else
		exports['gum_notify']:DisplayLeftNotification(Config.Language[1].text, Config.Language[4].text, 'money', 3000)
	end
end)
...
RegisterNUICallback('borrow_info', function(data, cb)
	exports['gum_notify']:DisplayLeftNotification(Config.Language[1].text, Config.Language[5].text, 'money', 5000)
end)
